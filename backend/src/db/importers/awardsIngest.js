...
function buildRawVendorPatch(vendor) {
  return compactObject({
    ...
    congressional_district: vendor.congressionalDistrict,
...
function mapUploadedRow(rawRow) {
  ...
  const vendor = {
    ...
    congressionalDistrict: pickFirst(rawRow, [
      'congressionalDistrict',
      'vendorData.domestic_vendor.address.congressional_district',
      'vendorData.parent_vendor.address.congressional_district',
      'vendorData.global_vendor.address.congressional_district',
    ]),
  ...
  const award = {
    ...
    numberOfOffersReceived: parseInteger(pickFirst(rawRow, [
      'awardData.referenced_idv.number_of_offers_received',
      'awardData.number_of_offers_received',
      'awardData.number_of_offers_recieved',
    ])),
    placeOfPerformanceCity: pickFirst(rawRow, ['acquisitionData.place_of_performance.city.name']),
    placeOfPerformanceCongressionalDistrict: pickFirst(rawRow, ['acquisitionData.place_of_performance.congressional_district']),
    placeOfPerformanceCountyCode: pickFirst(rawRow, ['acquisitionData.place_of_performance.county.code'], { upper: true }),
    placeOfPerformanceCountyName: pickFirst(rawRow, ['acquisitionData.place_of_performance.county.name']),
  ...
  award.extraAttributes = compactObject({
    ...
    number_of_offers_received: award.numberOfOffersReceived,
    congressional_district: vendor.congressionalDistrict,
    place_of_performance_congressional_district: award.placeOfPerformanceCongressionalDistrict,
    place_of_performance_county_code: award.placeOfPerformanceCountyCode,
    place_of_performance_county_name: award.placeOfPerformanceCountyName,
  });
...
function prepareBulkRow(rawRow, sourceRowNumber) {
  ...
  return {
    ...
    row: {
      ...
      congressional_district: mapped.vendor.congressionalDistrict,
      ...
      number_of_offers_received: mapped.award.numberOfOffersReceived,
      place_of_performance_city: mapped.award.placeOfPerformanceCity,
      place_of_performance_congressional_district: mapped.award.placeOfPerformanceCongressionalDistrict,
      place_of_performance_county_code: mapped.award.placeOfPerformanceCountyCode,
      place_of_performance_county_name: mapped.award.placeOfPerformanceCountyName,
...
updated_vendors AS (
  UPDATE vendor_entities ve
     SET vendor_name = COALESCE(NULLIF(mvr.vendor_name, ''), ve.vendor_name),
         ...
         congressional_district = COALESCE(NULLIF(mvr.congressional_district, ''), ve.congressional_district),
...
upsert_awards AS (
  ...
  ON CONFLICT (award_key) DO UPDATE
    SET ingest_file_id = COALESCE(EXCLUDED.ingest_file_id, award_transactions.ingest_file_id),
        ...
        number_of_offers_received = COALESCE(EXCLUDED.number_of_offers_received, award_transactions.number_of_offers_received),
        ...
        place_of_performance_congressional_district = COALESCE(NULLIF(EXCLUDED.place_of_performance_congressional_district, ''), award_transactions.place_of_performance_congressional_district),
        place_of_performance_county_code = COALESCE(NULLIF(EXCLUDED.place_of_performance_county_code, ''), award_transactions.place_of_performance_county_code),
        place_of_performance_county_name = COALESCE(NULLIF(EXCLUDED.place_of_performance_county_name, ''), award_transactions.place_of_performance_county_name),
    ...
    const updateResult = await client.query(
      `UPDATE vendor_entities
          SET vendor_key = $2,
              ...
              congressional_district = COALESCE(NULLIF($19, ''), congressional_district),
              state_code = COALESCE(NULLIF($20, ''), state_code),
              state_name = COALESCE(NULLIF($21, ''), state_name),
              zip_code = COALESCE(NULLIF($22, ''), zip_code),
              country_code = COALESCE(NULLIF($23, ''), country_code),
              country_name = COALESCE(NULLIF($24, ''), country_name),
              source_vendor_info_id = COALESCE(NULLIF($25, ''), source_vendor_info_id),
              source_vendor_history_id = COALESCE(NULLIF($26, ''), source_vendor_history_id),
              raw_vendor = $27::jsonb
        WHERE vendor_id = $1
        RETURNING vendor_id`,
      [
...
async function upsertAwardTransaction(client, ingestFileId, rawAwardRowId, vendorId, mapped) {
  ...
  const result = await client.query(
    ...
    [
      ...
      award.placeOfPerformanceCongressionalDistrict,
      award.placeOfPerformanceCountyCode,
      award.placeOfPerformanceCountyName,
